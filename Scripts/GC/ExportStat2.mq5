...
void OnStart()
  {
  CPInit();   Write_File(90000,_Pers_); //
   ...
   if(FileHandle!=INVALID_HANDLE)
     {
      ...
      FileWrite(FileHandle,// записываем в файл шапку
                "Symbol","DayOfWeek","Hours","Minuta","QtyBuy","QtySell","QtyWait");//,(string)tm.day+"/"+(string)tm.mon+"/"+(string)tm.year);
      ...
      for(SymbolIdx=0; SymbolIdx<MaxSymbols;SymbolIdx++)
        {
         ...
         for(i=0;i<qty;i++)
           {
            res=GetTrend(10,SymbolsArray[SymbolIdx],PERIOD_M1,i,false);
              {
                outstr="";
...
//                  FileWrite(TrainFile,IV[0],IV[1],res);
 //                 FileWrite(TrainFile,outstr);
               ...
               FileWrite(FileHandle,// записываем в файл шапку
                SymbolsArray[SymbolIdx],tm.day_of_week,tm.hour,tm.min,QtyBuy,QtySell,QtyWait);//,(string)tm.day+"/"+(string)tm.mon+"/"+(string)tm.year);
