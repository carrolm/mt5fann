...
input int    InpTrailingStop  =40;  // Trailing Stop Level (in pips)
input int    InpMACDOpenLevel =3;   // MACD open level (in pips)
input int    InpMACDCloseLevel=2;   // MACD close level (in pips)
input int    InpRSITrendPeriod=160;  // MA trend period
//input int    inpMACD1 =48;
//input int    inpMACD2 =36;
//input int    inpMACD3 =19;
...
protected:
   ...
   //--- indicators
   int               m_handle_RSI;                // MACD indicator handle
...
//--- create MACD indicator
   if(m_handle_macd==INVALID_HANDLE)
      if((m_handle_macd=iMACD(NULL,0,inpMACD1,inpMACD2,inpMACD3,PRICE_CLOSE))==INVALID_HANDLE)
        {
...
//--- refresh rates
   if(!m_symbol.RefreshRates())
      return(false);
//--- refresh indicators
   if(BarsCalculated(m_handle_RSI)<2)
      return(false);
   if(CopyBuffer(m_handle_RSI,0,0,2,m_buff_RSI)  !=2 )//||
...
//--- don't process if timeout
   if(TimeCurrent()>=limit_time)
     {
      //--- check for data
      if(Bars(Symbol(),Period())>2*InpRSITrendPeriod)
        {
